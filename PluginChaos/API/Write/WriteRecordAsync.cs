...
namespace PluginChaos.API.Write
{
    public static partial class Write
    {
        private static readonly SemaphoreSlim WriteSemaphoreSlim = new SemaphoreSlim(1, 1);

        public static async Task<string> WriteRecordAsync(Schema schema, Record record,
            IServerStreamWriter<RecordAck> responseStream)
        {
            ...
            try
            {
                ...
                // write records
                var errorMessage = await endpoint.WriteRecordAsync(schema, record, responseStream);
